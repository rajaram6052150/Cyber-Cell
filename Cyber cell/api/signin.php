<?php
header("Access-Control-Allow-Origin:*");
header("Access-Control-Allow-Credentials:true");
header('Access-Control-Allow-Methods: POST, GET, DELETE, PUT');
header("Access-Control-Allow-Headers:Access-Control-Allow-Headers, Origin,Accept, X-Requested-With, Content-Type, Access-Control-Request-Method, Access-Control-Request-Headers");

include 'jwt/JWT.php';

use Firebase\JWT\JWT;

if ($_POST['username'] != '') {
    date_default_timezone_set('Asia/Kolkata');

    include 'config.php';
    $resp_status = new stdClass;
    $privateKey = <<<EOD
-----BEGIN RSA PRIVATE KEY-----

AOBTZbyuwbm6FE9XGaX+v4xABfveMtReDp+K3HkHjJR8gipMYyO7l7XC
-----END RSA PRIVATE KEY-----
EOD;

    $username = $_POST['username'];
    $password = $_POST['password'];

    $check = $dbcon->query("SELECT * from users where username='$username' and `password`='$password';");

    if ($check->num_rows > 0) {
        $result=mysqli_fetch_assoc($check);
        $iat = time();
        $nbf = $iat + 0;
        $exp = $iat + 7200;
        $payload = [
            'iat' => $iat,
            'nbf' => $nbf,
            'exp' => $exp,
            "data" => [
                "email" => $result['email'],
                "name" => $result['name']
            ]
        ];
        $token = JWT::encode($payload, $privateKey, 'RS256');
        $dbcon->query("update users set token='$token' where id='$result[id]';");

        $resp_status->status = 'ok';
        $resp_status->token = $token;
        $resp_status->expire = $exp;
        $resp_status->name=$result['name'];
        $resp_status->email=$result['email'];
        $resp_status->email=$result['username'];

        echo json_encode($resp_status);
    } else {
        $resp_status->status = 'error';
        $resp_status->message = 'Invalid Credentials';
        echo json_encode($resp_status);
    }
    mysqli_close($dbcon);
}
