...
$response = [];
    ...
    $check = $dbcon->query("SELECT * FROM excel_data WHERE request_pending='Y' ORDER by ack_no DESC;");
    if ($check->num_rows > 0) {
            $result = mysqli_fetch_assoc($check);
            $dbcon->query("UPDATE excel_data SET approved='Y' where request_pending='Y';");
        mysqli_close($dbcon);
        }

else
{
    $check = $dbcon->query("SELECT * FROM excel_data WHERE approved='Y' ORDER by ack_no DESC;");
    if ($check->num_rows > 0) {
